...
if($_POST)
{
    ...
    if($connexion->rowCount() != 0) // si le resultat est différent de 0, c'est que le pseudo est connu en BDD
    {
        ...
        {
            $content .= '<div class="alert alert-success col-md-8 col-md-offset-2 texte-center">ACHETE !!</div>';
            foreach($membre as $indice => $valeur) // on passe en revue les informations du membre qui a le bon mdp
            {
                ...
                {
                    $_SESSION['membre'][$indice] = $valeur; // on crée da,s le fichier session un tableau membre et on enregistre les données de l'internaute qui ppourra dès à présent naviguer sur le site sans être déconnecté
                }
            }
            //debug($_SESSION);
        ...
        {
            $content .= '<div class="alert alert-danger col-md-8 col-md-offset-2 texte-center">Mauvais mot de passe!</div>';
        }
    }
    else
    {
        $content .= '<div class="alert alert-danger col-md-8 col-md-offset-2 texte-center">Casse toi pauv\' con!</div>';
    }
}
